...
import com.raidentokado.rotp_bat.init.InitStands;
...
public class BMGrapplingVineEntity extends OwnerBoundProjectileEntity {
    ...
    public BMGrapplingVineEntity(World world, StandEntity entity, IStandPower userStand, boolean charg, LivingEntity suer) {
        super(InitEntities.BM_GRAPPLING_VINE.get(), entity, world);
        this.stand = entity;
    ...
    public BMGrapplingVineEntity(EntityType<? extends BMGrapplingVineEntity> entityType, World world) {
    ...
    protected void afterBlockHit(BlockRayTraceResult blockRayTraceResult, boolean brokenBlock) {
        if (!brokenBlock && !bindEntities) {
            if (!getBlockPosAttachedTo().isPresent()) {
                playSound(InitSounds.BATMAN_GRAPPLE_CATCH.get(), 1.0F, 1.0F);
